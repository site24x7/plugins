...
get_plugin_data() {
    ...
    tput setaf 3
    echo
    echo "------------Provide connection details to connect to Apache Flink------------"
    tput sgr0
    echo 
...
check_plugin_execution() {
    ...
    elif ! echo "$output" | grep -qE "\"taskmanager.memory.process.size\":|\"jobmanager.memory.off-heap.size\":"; then
        tput setaf 3
        echo "The output does not contain metrics. Check if you have provided the correct endpoint for the apache flink URL."
...
add_conf() {
    ...
    escaped_port=$(echo "$port" | sed 's/\\/\\\\/g')

    output=$(sed -i "/^host*/c\host = \"$escaped_host\"" $cfg_file)
    error_handler $? "$output"

    output=$(sed -i "/^port*/c\port = \"$escaped_port\"" $cfg_file)
    error_handler $? "$output"
