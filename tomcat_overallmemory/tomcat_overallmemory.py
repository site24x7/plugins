        ...
        if(serverinfoError==False):
            ...
            if (xmlError!=True):
                if (xmlTreeData!=None):
                    ...
                    data['free_memory']=convertBytesToMB(float(freeMem))
                    data['total_memory']=convertBytesToMB(float(totalMem))
                    data['max_memory']=convertBytesToMB(float(maxMem))
                    data['available_memory']=convertBytesToMB(float(availableMem))
                    data['used_memory']=convertBytesToMB(float(usedMem))
                    data['percent_used_memory']=percentUsedMem
                else:
