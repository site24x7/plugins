...
class cassandra:
    def __init__(self, args):
        ...
        self.port = args.port
        self.username = args.jmx_username
        self.password = args.jmx_password
    ...
    def metriccollector(self):
        try:
            ...
            if self.username and self.password:
                jmxConnection = jmx.JMXConnection(
                    f"service:jmx:rmi:///jndi/rmi://{self.hostname}:{self.port}/jmxrmi",
                    jmx_username=self.username,
            ...
            metric_queries={
                ...
                "Throughput (Writes)":"org.apache.cassandra.metrics:type=ClientRequest,scope=Write,name=Latency",
                "Throughput (Read)":"org.apache.cassandra.metrics:type=ClientRequest,scope=Read,name=Latency",
...
if __name__=="__main__":
    ...
    parser=argparse.ArgumentParser(description='Cassandra Monitoring Plugin')
    parser.add_argument('--hostname', help='host of cassandra',default=hostname)
    parser.add_argument('--port', help='JMX port of cassandra',default=port)
    parser.add_argument('--jmx_username', help='JMX username for authentication', default=username)
    parser.add_argument('--jmx_password', help='JMX password for authentication', default=password)
