...
public class Jira {
	...
	public static void main(String[] args) {
		try {
			...
			ObjectName query = new ObjectName("com.atlassian.jira:*");
	...
	private static void printJmxAttributes(Set<ObjectName> mbeans, MBeanServerConnection connection){
		...
		ignoreMetrics.add("BasicDataSource");
				...
				if(!isStringInArrayList(ignoreMetrics, mbean.toString())){
					...
					for (MBeanAttributeInfo attribute : attributes) {
						...
						if(isValue){
							...
							obj = connection.getAttribute(oname,attribute.getName());
							System.out.print("\n\t\"" + mbean.toString().substring("com.atlassian.jira:type=".length()) + "." + attribute.getName() + "\" : \"" + obj + "\"");
						}
					}
				}
			}
